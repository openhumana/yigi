...
(function(){
  ...
  // Intercept form submissions (e.g. Google Search) → route through proxy
  document.addEventListener('submit',function(e){
    ...
    e.preventDefault();e.stopPropagation();
    var absAction=new URL(action,location.href).href;
    ...
    var targetUrl=method==='post'?absAction:absAction.split('?')[0]+'?'+params.toString();
    try{window.parent.postMessage({type:'yogi-navigate',url:targetUrl},'*');}catch(x){}
  },true);
  // Intercept location assignments → route through proxy
  try{
    var origPushState=history.pushState.bind(history);
    var origReplaceState=history.replaceState.bind(history);
    history.pushState=function(s,t,url){if(url){try{window.parent.postMessage({type:'yogi-navigate',url:new URL(String(url),location.href).href},'*');}catch(x){}}};
    history.replaceState=function(s,t,url){if(url){try{window.parent.postMessage({type:'yogi-navigate',url:new URL(String(url),location.href).href},'*');}catch(x){}}};
  }catch(x){}
  ...
  window.addEventListener('message',function(e){
    ...
    if(d.type==='yogi-type'){el=document.querySelector(d.selector)||document.querySelector('input[name="q"]')||document.querySelector('input[type="search"]')||document.querySelector('textarea[name="q"]');if(el){el.focus();var desc=Object.getOwnPropertyDescriptor(Object.getPrototypeOf(el),'value');if(desc&&desc.set){desc.set.call(el,d.value);}else{el.value=d.value;}el.dispatchEvent(new Event('input',{bubbles:true,cancelable:true}));el.dispatchEvent(new Event('change',{bubbles:true,cancelable:true}));var isSearch=(el.type==='search'||el.getAttribute('role')==='searchbox'||(el.getAttribute('aria-label')||'').toLowerCase().includes('search')||(el.placeholder||'').toLowerCase().includes('search')||d.selector.toLowerCase().includes('search')||(el.name||'')==='q');if(isSearch){setTimeout(function(){el.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',code:'Enter',keyCode:13,bubbles:true,cancelable:true}));el.dispatchEvent(new KeyboardEvent('keypress',{key:'Enter',code:'Enter',keyCode:13,bubbles:true,cancelable:true}));el.dispatchEvent(new KeyboardEvent('keyup',{key:'Enter',code:'Enter',keyCode:13,bubbles:true,cancelable:true}));var form=el.closest('form');if(form){try{form.requestSubmit();}catch(x){form.dispatchEvent(new Event('submit',{bubbles:true,cancelable:true}));}}setTimeout(reportDOM,1500);},500);}else{setTimeout(reportDOM,500);}window.parent.postMessage({type:'yogi-typed',selector:d.selector,value:d.value},'*');}else{window.parent.postMessage({type:'yogi-typed',selector:d.selector,value:'',error:'element not found'},'*');}}
    if(d.type==='yogi-press-enter'){el=document.querySelector(d.selector)||document.activeElement;if(el){['keydown','keypress','keyup'].forEach(function(ev){el.dispatchEvent(new KeyboardEvent(ev,{key:'Enter',code:'Enter',keyCode:13,bubbles:true}));});var form=el.closest('form');if(form){try{form.requestSubmit();}catch(x){form.dispatchEvent(new Event('submit',{bubbles:true,cancelable:true}));}}setTimeout(reportDOM,1500);}}
